...
export function TitleEditor({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "prose prose-sm max-w-none focus:outline-none text-[#432817] px-3 py-2.5 text-[14px]",
        style:
          "font-family: var(--font-lato), sans-serif; min-height: 40px; max-height: 80px; overflow-y: auto;",
      },
    },
  });
...
export default function RichTextEditor({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "prose prose-sm max-w-none focus:outline-none text-[#432817] p-4 text-[14px]",
      ...
      <div className="flex flex-wrap items-center gap-0.5 px-3 py-2 border-b border-[#E0D5C5]/60 bg-[#FDFDFD]">
        ...
        <div className="relative" ref={headingMenuRef}>
          ...
          >
            ...
            <ChevronDown
              size={14}
              className={`transition-transform ${showHeadingMenu ? "rotate-180" : ""}`}
            />
          </button>
          {showHeadingMenu && (
